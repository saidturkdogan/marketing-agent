...
def linkedin_writer_node(state: AgentState):
    print("[LinkedInWriter]: building LinkedIn content package...")
    assets = state.get("assets", {})
    strategy = assets.get("strategy", {})
    research = assets.get("research", "")
    trend_report = assets.get("trend_report", "")
    specs = get_platform_specs.invoke({"platform": "linkedin"})
    ...
    return {
        "messages": build_message("LinkedInWriter", content),
        "assets": {"social": {"LinkedIn": content}},
        "completed_steps": ["LinkedInWriter"],
    }
